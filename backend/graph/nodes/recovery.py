...
def recovery_node(state: AgentState) -> dict:
    ...
    log.info("▶ recovery_node started")
        ...
        try:
            ...
            try:
                insert_row("actions_log", {
                    ...
                    "action": action,
                    "status": "escalated",
                    "details": {"error": str(exc), "max_retries": MAX_RETRIES},
                })
    ...
    log.info(f"▶ recovery_node complete — retry_count={retry_count}, escalated={context_memory.get('escalated', False)}")
    return {
