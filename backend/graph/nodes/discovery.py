...
def discovery_node(state: AgentState) -> dict:
    ...
    try:
        ...
        msg = f"discovery_node: CSV fetch/parse failed — {exc}"
        log.warning(msg)
        errors.append(msg)
    ...
    try:
        ...
        msg = f"discovery_node: PDF fetch/parse failed — {exc}"
        log.warning(msg)
        errors.append(msg)
    ...
    log.info(f"▶ discovery_node complete — {len(raw_data)} raw records")
    return {
        "raw_data": raw_data,
        "errors": errors,
    }
